...
**Insert profile views of drift and scour for high and low fidelity cases, for example 2012 v 2017 HV Polygons have high fidelity, CLPX lake 2012 vs. 2017 is low. Of course like drift threshold, this fidelity metrics depend on size of AOI**

## Discussion
The drift census (Table 3) confirms that drifts are important in at least the part of the Arctic where the study was conducted. There the drifts occupy 15% of the landscape, 30% of the snow volume, and about 40% of the SWE. These statistics would suggest that Arctic drifts are far more important than their areal fraction alone would suggest, and that in terms of hydrology, drifts should be considered to be 2 to 3 times more important that non-drift areas and therefor worthy of more attention. This is not a new finding, but lacking an operational and computational method of defining a drift (and mapping snow depth) few or no prior drift census exist. The method (Figure 6) for delineating drifts is novel and at least in this first application appears to define drifts in a way consistent with qualitative "eyeball tests." 

Differences in the general census by swath (HV vs. CLPX) indicate distinct differences in drift (Figure 7) that suggest an underlying control by physiography. These differences are particularly notable when we compute drift intensity, the ratio of the drift volume to the total area. Drift intensity could be thought of as the amount of snow from the region that has been sequestered in drifts, an amount with both hydrological and ecological significance. The swath differences suggested to us that we could divide the drifts into classes, some of the filling and some of the non-filling types. While similar separations have been made previously in the field and in profile view (Ekblaw, 1918; Lewis, 1939), through classification we can get at different snow stockpiling behaviors (depth distribution curves), forms, and geometries, thereby linking the planimetric view of drifts back to the profile view that dominated early transportation studies. These snowdrift classes are also useful archetypes for refining the linkages between snowdrift (and scour), wind transport, and topography (the snowdrift triad, Figure 2).

We then examined the year-to-year fidelity of drifts, which as others before us, we found to be exceptionally high (Table 5). But that examination also revealed that filling drifts have superior fidelity to nonfilling drifts, and that scour zones are the most changeable with respect to snow and fidelity (Figure 18). In hindsight, this finding could have been anticipated by thinking of a drift trap as a black hole: anything falling beyond the trap lip (event horizon) is permanently locked up and removed from the system. On the other hand, a scour zone can be located in many different locations while still sourcing a drift. Thus, small variations in winter wind directions and strengths alter where the scour areas fall, but not the drifts themselves.

Each class will typically exhibit *f* or *nf* properties within a field area: lake, stream, and outcrop drifts are of the *nf* type and so while they may be deeper, they tend to be fewer in number. Water track and ice wedge drifts (*f* type) do not stockpile as much snow, but occur in networks at greater areal density. The *f*-*nf* division is also seen in the year-to-year fidelity of the drifts. The *f* type drifts always have enough flux to fill and thus they repeat with extremely high fidelity, while the magnitude of *nf* drifts are flux dependent and thus they are more sensitive to the variability of each winter and have lesser fidelity compared to *f* drifts. The physical scale of the snowdrift classes has breaks (i.e. the classes do not scale linearly in size), and so it is likely beyond the bounds of the climate regime to have such a dramatic change in flux that causes a *f* to *nf* transition, or vice versa. However, it is possible that a "Goldilocks" drift is out there that is just the right size such that in some winters it is *f* and in others *nf*.

But for all this detail, and while relative differences in fidelity exist between snowdrift classes, the absolute level of fidelity within the snow depth maps is remarkably high (Table 5). The tundra snow depth patterns repeat each year with drift and scour elements arranged in much the same way. How is this constancy possible? Of the snowdrift triad (Figure 2) there are inherent amounts of variability: for simplicity, assume the top and left vertices (wind transport and available snow) are functions of the winter weather (ignoring their alteration by topography). Although for our field area (and for much of the snowdrift world) metereological data is not available, we know they are broadly bound by climate: the length of winter, number of storms, and prevailing winds. Within these climatic bounds there is substantial variability: the trajectory of a single winter (the amounts and timings of *P*, *T*, and *S* events) is impossible to predict. The top and left vertices of the triad move - but the movements cumulate and converge on *f* and *nf* drift classes and produce a snow pattern of drift and scour that is essentially the same each year. This convergence occurs because the right vertex, topography, anchors the the triad. Landscape filters the variability of each winter and funnels the unpredictable trajectories toward a convergent, cumulative result: the snow patterns and snowdrift classes seen here. Convergence of snow patterns has a rich history (Matthes, 1900; Yukigata; Bloschl; Sturm & Wagner, 2010), and now the length of our time series, large number of ground validation points, and fidelity metrics show that convergence is perhaps the rule, not the exception, and that in the Arctic where drifts are particularly long-lying, the snow depth patterns of various *nf* and *f* drifts (and scour) **are** the landscape.
